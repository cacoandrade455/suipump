...
// StrategiesModal.jsx
// Strategy hooks (useSniper, useDCA, useCopyTrade) are NO LONGER called here.
// They are passed in as props from App.jsx where they live permanently,
// so closing the modal does NOT stop the strategies.
...
import { DEFAULT_SNIPER_CONFIG } from './useSniper.js';
    ...
    <div className="space-y-4">
      <div className="space-y-1">
        ...
        <p className="text-[10px] font-mono text-white/40 leading-relaxed">
          A dedicated hot wallet keypair for autonomous strategy execution — signs sniper/DCA/copy trades without Slush popups.
          Strategies now run in the background even with the modal closed.
        </p>
        <p className="text-[9px] font-mono text-white/20 leading-relaxed">
          Key is encrypted with your Slush signature and stored in localStorage. Clearing your browser data will remove it.
        </p>
      </div>
...
// ── Sniper tab ────────────────────────────────────────────────────────────────
// Receives the sniper hook instance from App.jsx — does NOT call useSniper itself.
function SniperTab({ sniper, hasKey, isReady }) {
  const account = useCurrentAccount();
  const { config, updateConfig, enable, disable, log, clearLog, sniping, isActive } = sniper;

  const handleToggle = () => { if (isActive) disable(); else enable(); };
  ...
  if (!hasKey || !isReady) return (
    <div className="py-12 text-center space-y-2">
      ...
      <div className="text-[9px] font-mono text-white/15">Set up your trading key in the Key tab → it signs snipe transactions autonomously</div>
    </div>
  );
  ...
  return (
    <div className="space-y-4">
      ...
      <div className="space-y-3">
        <div className="text-[9px] font-mono text-white/30 tracking-widest">SNIPER CONFIG</div>

        <div className="grid grid-cols-2 gap-2">
          <div className="space-y-1">
            <label className="text-[9px] font-mono text-white/30">MAX SUI PER SNIPE</label>
            <input
              type="number" min="0.1" step="0.1"
              value={config.maxSuiPerSnipe ?? 1}
        ...
        <div className="space-y-1">
          <label className="text-[9px] font-mono text-white/30">MAX DEV BUY %</label>
          <input
            type="number" min="0" max="100" step="1"
            value={config.maxDevBuyPct ?? 20}
            onChange={e => updateConfig({ maxDevBuyPct: parseFloat(e.target.value) || 20 })}
            className="w-full bg-white/[0.04] border border-white/8 rounded-lg px-3 py-1.5 text-[11px] font-mono text-white focus:outline-none focus:border-lime-400/30"
          />
          <p className="text-[8px] font-mono text-white/20">Skip tokens where dev bought more than this % of supply</p>
        </div>
      ...
      <div className="text-[8px] font-mono text-white/15 text-center">
        Sniper runs in the background · only stops when the browser tab is closed
      </div>
    </div>
  );
}
...
// ── DCA tab ───────────────────────────────────────────────────────────────────
// Receives the dca hook instance from App.jsx — does NOT call useDCA itself.
function DCATab({ dca, hasKey, isReady }) {
  const account = useCurrentAccount();
  ...
  const handleCreate = () => {
    ...
    const sui = parseFloat(totalSui);
    const n   = parseInt(tranches);
    if (!sui || sui <= 0) { setFormMsg('Enter total SUI'); return; }
    if (!n   || n   <= 0) { setFormMsg('Enter tranche count'); return; }
    createOrder({
      curveId, tokenType: resolvedType, pkgId: resolvedPkg,
      name: resolvedName, symbol: resolvedSym,
      totalSui: sui, trancheCount: n,
  ...
  return (
    <div className="space-y-4">
      ...
      {activeOrders.length > 0 && (
        <div className="space-y-2">
          <div className="text-[9px] font-mono text-white/30 tracking-widest">ACTIVE DCA ORDERS</div>
          {activeOrders.map(order => (
            <div key={order.id} className="rounded-xl border border-lime-400/15 bg-lime-950/10 px-3 py-2.5 flex items-center justify-between">
              <div className="min-w-0">
                <div className="text-[10px] font-mono font-bold text-white truncate">{order.name || order.curveId.slice(0,8)+'…'}</div>
                <div className="text-[8px] font-mono text-white/30">
                  {order.tranchesFired ?? 0}/{order.trancheCount} tranches · {order.totalSui} SUI total
                </div>
              </div>
      ...
      <div className="space-y-3">
        ...
        <div className="space-y-1">
          ...
          <input
            value={curveId}
            onChange={e => { setCurveId(e.target.value); resolveCurve(e.target.value.trim()); }}
            placeholder="0x… paste curve ID"
        ...
        <div className="grid grid-cols-2 gap-2">
          ...
          <div className="space-y-1">
            ...
            <input type="number" min="1" step="1" value={tranches}
        ...
        <div className="grid grid-cols-2 gap-2">
          <div className="space-y-1">
            ...
            <select value={intervalMs} onChange={e => setIntervalMs(e.target.value)}
              className="w-full bg-white/[0.04] border border-white/8 rounded-lg px-3 py-1.5 text-[11px] font-mono text-white focus:outline-none focus:border-lime-400/30">
        ...
        <button onClick={handleCreate}
          ...
          className={`w-full py-2.5 rounded-xl text-[11px] font-mono font-bold transition-colors ${
            !resolvedType ? 'bg-white/5 text-white/20 cursor-not-allowed' : 'bg-lime-400 text-black hover:bg-lime-300'
          }`}>
          START DCA
        </button>
      ...
      <div className="text-[8px] font-mono text-white/15 text-center">
        DCA runs in the background · only stops when the browser tab is closed
      </div>
    </div>
  );
}
...
// ── Copy Trade tab ────────────────────────────────────────────────────────────
// Receives the copyTrade hook instance from App.jsx — does NOT call useCopyTrade itself.
function CopyTradeTab({ copyTrade, hasKey, isReady }) {
  const account = useCurrentAccount();
  ...
  if (!hasKey || !isReady) return (
    <div className="py-12 text-center space-y-2">
      <div className="text-[11px] font-mono text-white/25">Trading key required</div>
      <div className="text-[9px] font-mono text-white/15">Set up your trading key in the Key tab → it mirrors trades autonomously</div>
    </div>
  );
  ...
  return (
    <div className="space-y-4">
        ...
        <div className="space-y-2">
          ...
          {targets.map(t => (
            <div key={t.address} className={`rounded-xl border px-3 py-2.5 flex items-center justify-between ${
              ...
              <div className="min-w-0">
                {t.label && <div className="text-[11px] font-mono font-bold text-white truncate">{t.label}</div>}
      ...
      {/* Add wallet form */}
      <div className="space-y-3">
        <div className="text-[9px] font-mono text-white/30 tracking-widest">WATCH A WALLET</div>
        ...
        <div className="flex items-center justify-between">
          <span className="text-[10px] font-mono text-white/40">Mirror sells</span>
        ...
        <button onClick={handleAdd}
          className="w-full py-2.5 rounded-xl bg-lime-400 text-black text-[11px] font-mono font-bold hover:bg-lime-300 transition-colors">
          ADD WALLET
        </button>
        {formMsg && (
          <div className={`text-[10px] font-mono text-center ${formMsg.includes('✓') ? 'text-lime-400' : 'text-red-400'}`}>
      ...
      <div className="text-[8px] font-mono text-white/15 text-center">
        Copy trading runs in the background · only stops when the browser tab is closed
      </div>
    </div>
  );
}
...
function ActiveStrategiesTab() {
  ...
  if (!account) return (
    <div className="py-12 text-center text-[11px] font-mono text-white/30">Connect your wallet to view active strategies</div>
  );

  if (!tpslConfigs.length) return (
    <div className="py-12 text-center space-y-2">
      <div className="text-[11px] font-mono text-white/20">No active TP/SL configs</div>
      <div className="text-[9px] font-mono text-white/15">Set TP/SL on a token page to see it here</div>
    </div>
  );
      ...
      {tpslConfigs.map(({ curveId, config }) => (
        <div key={curveId} className="rounded-xl border border-white/8 bg-white/[0.02] px-3 py-2.5">
          <div className="flex items-center justify-between mb-2">
            <div className="text-[10px] font-mono text-white/60 truncate">{curveId.slice(0,12)}…</div>
      ...
      >
        ...
        <div className="flex items-center justify-between px-5 py-4 border-b border-white/8">
          <div className="flex items-center gap-2">
            ...
            {(sniper?.isActive || dca?.activeOrders?.length > 0 || copyTrade?.isActive) && (
        ...
        <div className="px-5 py-5 max-h-[70vh] overflow-y-auto">
          ...
          {tab === 'sniper' && <SniperTab    sniper={sniper}       hasKey={hasKey} isReady={isReady} />}
          {tab === 'dca'    && <DCATab       dca={dca}             hasKey={hasKey} isReady={isReady} />}
          {tab === 'copy'   && <CopyTradeTab copyTrade={copyTrade} hasKey={hasKey} isReady={isReady} />}
