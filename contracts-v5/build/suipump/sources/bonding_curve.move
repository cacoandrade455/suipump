/// suipump::bonding_curve v5
///
/// Changes from v4:
///   - VIRTUAL_SUI_RESERVE lowered: 30,000 → 9,000 SUI (~$10k starting mcap at $1.21/SUI)
///   - GRAD_THRESHOLD_MIST lowered: 35,000 → 17,000 SUI (~$20k USD to DEX pool)
///   - graduation_target: u8 on Curve struct (0=Cetus, 1=DeepBook)
///   - anti_bot_delay: u8 on Curve struct (0/15/30 seconds, creator's choice)
///   - referral: Option<address> on buy() — 5-way fee split when set
///   - CreatorCap metadata updates with 24h timelock (name/symbol/description/icon)
///   - update_coin_metadata() for admin to push metadata post-timelock
///
...
module suipump::bonding_curve {
    ...
    const ETimelockNotExpired:      u64 = 21;
    const ENoMetadataUpdate:        u64 = 22;
    const EMetadataNameTooLong:     u64 = 23;
    const EMetadataSymbolTooLong:   u64 = 24;
    ...
    const REFERRAL_SHARE_BPS:     u64 = 1_000; // carved from protocol when referral set
    ...
    // V5: 9,000 SUI virtual reserve → ~$10k starting mcap at $1.21/SUI
    const VIRTUAL_SUI_RESERVE:    u64 = 9_000 * 1_000_000_000;
    // Vt unchanged — proven curve shape
    const VIRTUAL_TOKEN_RESERVE:  u64 = 1_073_000_000 * 1_000_000;

    // V5: 17,000 SUI graduation → ~$20k USD to DEX pool at $1.21/SUI
    // Triggers at ~86.8% of curve sold. Changeable via UpgradeCap.
    ...
    // Metadata timelock: 24 hours in milliseconds
    const METADATA_TIMELOCK_MS: u64 = 24 * 60 * 60 * 1_000;
    ...
    public struct PendingMetadata has store, drop {
        name:        Option<String>,
        symbol:      Option<AsciiString>,
        description: Option<String>,
        icon_url:    Option<AsciiString>,
        unlocks_at:  u64, // epoch ms
    }
    ...
    public struct Curve<phantom T> has key {
        ...
        symbol:            AsciiString,
        // V5 fields
        graduation_target: u8,            // 0=Cetus, 1=DeepBook
        anti_bot_delay:    u8,            // 0, 15, or 30 seconds
        created_at_ms:     u64,           // clock timestamp at launch
    ...
    public struct MetadataUpdateQueued has copy, drop {
        curve_id:   ID,
        unlocks_at: u64,
    }

    public struct MetadataUpdateApplied has copy, drop {
        curve_id: ID,
    }

    // ---------- Creation ----------
    /// Legacy single-creator launch. No launch fee. No v5 features.
    public fun create<T>(
    ...
    /// Primary launch entry point. Returns (Curve, CreatorCap) for PTB dev-buy.
    public fun create_and_return<T>(
        ...
        // Store description in name field suffix via || delimiter (existing convention)
        // The full description string is passed through but stored off-chain via event
        let _ = description; // used by indexer via CurveCreated event args
    ...
    public fun buy<T>(
        ...
        // Anti-bot: block non-creator buys during delay window
        ...
        // Referral paid directly to referral address
    ...
    // ---------- Graduation ----------
    /// Permissionless — anyone can trigger when sui_reserve >= GRAD_THRESHOLD_MIST.
    /// graduation_target stored on curve for the PTB script to read and route.
    #[allow(lint(self_transfer))]
    public fun graduate<T>(
        ...
        // Creator bonus: 0.50% of final reserve
        ...
        // LP tokens → creator (for DEX pool composition)
        transfer::public_transfer(
    ...
    /// Admin claims pool SUI post-graduation for DEX pool composition.
    ...
    // ---------- Metadata updates (24h timelock) ----------
    /// Creator queues a metadata update. Executes after 24h via apply_metadata_update().
    public fun queue_metadata_update<T>(
        cap:         &CreatorCap,
        curve:       &mut Curve<T>,
        name:        Option<String>,
        symbol:      Option<AsciiString>,
        ...
        let now_ms     = clock::timestamp_ms(clock);
        let unlocks_at = now_ms + METADATA_TIMELOCK_MS;

        curve.pending_metadata = option::some(PendingMetadata {
            name,
            symbol,
            description,
            icon_url,
        ...
        event::emit(MetadataUpdateQueued {
            curve_id:   object::id(curve),
            unlocks_at,
        });
    }

    /// Permissionless execution after timelock expires.
    /// Requires CoinMetadata object to push updates on-chain.
    public fun apply_metadata_update<T>(
        curve:    &mut Curve<T>,
        metadata: &mut CoinMetadata<T>,
        clock:    &Clock,
        _ctx:     &mut TxContext,
    ) {
        assert!(option::is_some(&curve.pending_metadata), ENoMetadataUpdate);
        let pending = option::borrow(&curve.pending_metadata);
        let now_ms  = clock::timestamp_ms(clock);
        assert!(now_ms >= pending.unlocks_at, ETimelockNotExpired);

        // Apply each field if set
        if (option::is_some(&pending.name)) {
            coin::update_name(&curve.treasury, metadata, *option::borrow(&pending.name));
        };
        if (option::is_some(&pending.symbol)) {
            coin::update_symbol(&curve.treasury, metadata, *option::borrow(&pending.symbol));
        };
        if (option::is_some(&pending.description)) {
            coin::update_description(&curve.treasury, metadata, *option::borrow(&pending.description));
        };
        if (option::is_some(&pending.icon_url)) {
            let url = std::ascii::into_bytes(*option::borrow(&pending.icon_url));
            coin::update_icon_url(&curve.treasury, metadata, std::ascii::string(url));
        };

        curve.pending_metadata = option::none();

        event::emit(MetadataUpdateApplied { curve_id: object::id(curve) });
    }
    ...
    public fun has_pending_metadata<T>(c: &Curve<T>): bool {
        option::is_some(&c.pending_metadata)
    }

    public fun pending_metadata_unlocks_at<T>(c: &Curve<T>): u64 {
        if (option::is_some(&c.pending_metadata)) {
            option::borrow(&c.pending_metadata).unlocks_at
        } else {
            0
        }
    }
    ...
    #[test_only] public fun e_timelock_not_expired(): u64   { ETimelockNotExpired }
}
