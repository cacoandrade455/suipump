...
#                     JSON-RPC PURGE FLAG (2026-07-20, owner decision required):
#                     this default endpoint is DEAD (public fullnode JSON-RPC
#                     retired; hard shutdown 2026-07-31). It feeds ONLY the
#                     external Rust nexus CLI below, which speaks JSON-RPC
#                     internally and cannot use GraphQL. The Nexus/Leader
#                     execution path is SCOPED OUT of v1 (bridge-direct is the
#                     settled decision), so the `nexus conf set` step may
#                     already be failing silently. Options: (a) leave as-is,
#                     (b) set SUI_RPC_URL on Render to a live third-party
#                     JSON-RPC endpoint, (c) strip the nexus CLI configuration
#                     from this script entirely. Do NOT point SUI_GRAPHQL_URL
#                     at it - the CLI does not speak GraphQL. server.js and
#                     strategy.js already use SUI_GRAPHQL_URL and are
...
if [ -z "${SUI_RPC_URL:-}" ]; then
  echo "[start] WARNING: SUI_RPC_URL not set; nexus CLI will be configured with the DEAD default $RPC_URL (public JSON-RPC retired, shutdown 2026-07-31). Nexus/Leader path is scoped out of v1; see the JSON-RPC PURGE FLAG comment above." >&2
fi
