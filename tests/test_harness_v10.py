...
class Curve:
    ...
    # ---------- V10 ----------
    buyback_bps: int = 0              # carved from creator slice (0..=10000)
    buyback_burn: bool = False        # True=burn, False=return to creator
    buyback_fees: int = 0             # accrued SUI awaiting execute_buyback
    active_creator_cap_id: str = "CAP_0"
    last_creator_activity_ms: int = 0
    cto_cooldown_until_ms: int = 0
...
def buy(c, sui_in, min_tokens_out, buyer, price_scaled=0, referral=None):
    ...
    buyback = cf * c.buyback_bps // BPS_DENOMINATOR
    cf = cf - buyback
    c.buyback_fees += buyback
    ...
    # Move: payment after splits = sui_in - cf - pf - af - rf - buyback - swap_amount
    coin_refund = sui_in - cf - pf - af - rf - buyback - swap_amount
...
# ---------- V10 helpers ----------
def set_buyback_config(c, buyback_bps, burn):
    if buyback_bps > BPS_DENOMINATOR:
        raise MoveAbort(38)  # E_BUYBACK_BPS_TOO_HIGH
    c.buyback_bps = buyback_bps
    c.buyback_burn = burn


def execute_buyback(c):
    """Spend the whole buyback bucket to buy curve tokens; burn or return.
    Returns (sui_spent, tokens_bought)."""
    if c.graduated:
        raise MoveAbort(E_ALREADY_GRADUATED)
    spend = c.buyback_fees
    if spend == 0:
        raise MoveAbort(39)  # E_NO_BUYBACK
    x = c.eff_sui(); y = c.eff_token()
    remaining = c.token_reserve
    naive = quote_out(spend, x, y)
    tokens_out = min(naive, remaining)
    if tokens_out == 0:
        raise MoveAbort(39)
    # buyback SUI enters the real reserve (it bought tokens)
    c.buyback_fees -= spend
    c.sui_reserve += spend
    # tokens leave the reserve; burn (gone) or return to creator (exits model)
    c.token_reserve -= tokens_out
    return spend, tokens_out


def circulating_supply(c):
    return CURVE_SUPPLY - c.token_reserve
...
# V10 TESTS
# ═══════════════════════════════════════════════════════════════════════════
print("\n── V10: buyback (carved from creator slice) ──")

@test("buyback off: creator slice unchanged")
def _():
    c = Curve(creator=CREATOR)
    sui_in = 100 * MIST_PER_SUI
    _, (cf, pf, af, lp, rf), refund = buy(c, sui_in, 0, BUYER)
    # 100 SUI buy, fee=1 SUI, creator slice=40% of fee=0.4 SUI, no buyback.
    assert c.buyback_fees == 0
    assert cf == 400_000_000   # 0.4 SUI to creator, nothing carved

@test("buyback 50%: half the creator slice carved into bucket")
def _():
    c = Curve(creator=CREATOR)
    set_buyback_config(c, 5_000, True)   # 50% of creator slice
    sui_in = 100 * MIST_PER_SUI
    _, (cf, pf, af, lp, rf), refund = buy(c, sui_in, 0, BUYER)
    # creator slice 0.4 SUI -> 0.2 to creator, 0.2 to buyback bucket
    assert cf == 200_000_000, f"cf={cf}"
    assert c.buyback_fees == 200_000_000, f"bucket={c.buyback_fees}"

@test("buyback conservation: total still equals sui_in")
def _():
    c = Curve(creator=CREATOR)
    set_buyback_config(c, 3_333, False)  # arbitrary bps, return mode
    sui_in = 137 * MIST_PER_SUI          # arbitrary, exercises rounding
    _, (cf, pf, af, lp, rf), refund = buy(c, sui_in, 0, BUYER)
    # Full conservation INCLUDING the carved buyback bucket:
    #   cf + buyback + pf + af + swap_to_reserve + refund == sui_in
    total = cf + c.buyback_fees + pf + af + c.sui_reserve + rf + refund
    assert abs(total - sui_in) <= 2, f"total={total} sui_in={sui_in} diff={total-sui_in}"

@test("buyback 100%: entire creator slice carved")
def _():
    c = Curve(creator=CREATOR)
    set_buyback_config(c, 10_000, True)
    sui_in = 100 * MIST_PER_SUI
    _, (cf, pf, af, lp, rf), refund = buy(c, sui_in, 0, BUYER)
    assert cf == 0, f"cf should be 0, got {cf}"
    assert c.buyback_fees == 400_000_000

@test("buyback bps > 10000 rejected")
def _():
    c = Curve(creator=CREATOR)
    try:
        set_buyback_config(c, 10_001, True)
        assert False, "should have aborted"
    except MoveAbort as e:
        assert e.code == 38

@test("execute_buyback drains bucket and removes tokens")
def _():
    c = Curve(creator=CREATOR)
    set_buyback_config(c, 10_000, True)
    buy(c, 100 * MIST_PER_SUI, 0, BUYER)
    ...
    tok_before = c.token_reserve
    spent, bought = execute_buyback(c)
    assert c.buyback_fees == 0
    assert c.sui_reserve == reserve_before + spent  # buyback SUI entered reserve
    assert c.token_reserve == tok_before - bought   # tokens removed (burned)
    assert bought > 0

@test("execute_buyback with empty bucket aborts")
def _():
    c = Curve(creator=CREATOR)
    try:
        execute_buyback(c)
...
@test("circulating supply excludes curve token_reserve")
def _():
    c = Curve(creator=CREATOR)
    assert circulating_supply(c) == 0           # nothing sold yet
    tokens, _, _ = buy(c, 1_000 * MIST_PER_SUI, 0, BUYER)
    assert circulating_supply(c) == tokens      # exactly what left the curve

@test("nominate threshold: 1% of circulating")
def _():
    c = Curve(creator=CREATOR)
    buy(c, 5_000 * MIST_PER_SUI, 0, BUYER)
    circ = circulating_supply(c)
    threshold = circ * 100 // BPS_DENOMINATOR    # CTO_NOMINATE_BPS = 100
    # a holder with exactly threshold passes; threshold-1 fails
    assert threshold > 0
    assert (threshold >= threshold)              # >= passes
    assert not (threshold - 1 >= threshold)      # below fails

@test("quorum: for+against must exceed 25% of circulating (strict)")
def _():
    circ = 1_000_000_000
    quorum = circ * 2_500 // BPS_DENOMINATOR     # 25%
    # exactly quorum FAILS (strict >), quorum+1 passes
    assert not (quorum > quorum)
    assert (quorum + 1 > quorum)

@test("pass condition: for > against strict; tie favors incumbent")
def _():
    # tie -> not passed
    for_w, against_w = 500, 500
    assert not (for_w > against_w)
    # majority -> passed (assuming quorum met)
    for_w, against_w = 600, 400
    assert (for_w > against_w)

print("\n── V10: protocol surcharge ──")

@test("collect_protocol_surcharge adds to protocol_fees")
def _():
    c = Curve(creator=CREATOR)
    before = c.protocol_fees
    # mirror: collect_protocol_surcharge just joins payment into protocol_fees
    surcharge = 2 * MIST_PER_SUI
    c.protocol_fees += surcharge
    assert c.protocol_fees == before + surcharge
