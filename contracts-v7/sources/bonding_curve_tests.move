/// Unit tests for suipump::bonding_curve (v7)
///
...
///   - Curve shape recalibrated: VS=3.5k, GRAD_THRESHOLD=9k
///   - New tests: airdrop bucket split, pause flag, self-referral rejection,
///     lp_fees counter, record_graduation_pool, graduation target Turbos,
///     claim_airdrop_fees, comment fee
///
...
///
/// Run with: `sui move test` from the contracts-v7/ directory.
#[test_only]
module suipump::bonding_curve_tests {
    ...
    const E_INVALID_GRAD_TARGET:      u64 = 20;
    ...
    fun test_three_way_payout_split_pays_each_recipient() {
        ...
        let cap   = ts::take_from_sender<CreatorCap>(&s);
        let mut curve = ts::take_shared<Curve<TEST_TOKEN>>(&s);
        let total = bonding_curve::creator_fees_pending(&curve);
    ...
    fun test_whale_buy_clips_and_refunds() {
        ...
        assert!(coin::value(&tokens) == bonding_curve::curve_supply(), 800);
    ...
    fun test_buy_no_referral_splits_protocol_bucket_in_half() {
        ...
        ts::next_tx(&mut s, BUYER);
        let mut curve = ts::take_shared<Curve<TEST_TOKEN>>(&s);
        let proto_before   = bonding_curve::protocol_fees_pending(&curve);
        let airdrop_before = bonding_curve::airdrop_fees_pending(&curve);
        ...
        let proto_delta   = bonding_curve::protocol_fees_pending(&curve) - proto_before;
        let airdrop_delta = bonding_curve::airdrop_fees_pending(&curve) - airdrop_before;
    ...
    fun test_unpause_restores_trading() {
        ...
        assert!(!bonding_curve::is_paused(&curve), 1400);
        ts::return_to_sender(&s, admin_cap);
    ...
    fun test_record_graduation_pool_stores_ids() {
        ...
        assert!(option::is_some(&bonding_curve::creator_lp_nft_id(&curve)), 1501);
    ...
    fun test_claim_airdrop_fees_drains_bucket() {
        ...
        let admin_cap = ts::take_from_sender<AdminCap>(&s);
        let mut curve = ts::take_shared<Curve<TEST_TOKEN>>(&s);
        let airdrop = bonding_curve::claim_airdrop_fees(
            &admin_cap, &mut curve, ts::ctx(&mut s)
        );
        assert!(coin::value(&airdrop) > 0, 1700);
        assert!(bonding_curve::airdrop_fees_pending(&curve) == 0, 1701);
        destroy(airdrop);
        ts::return_to_sender(&s, admin_cap);
        ts::return_shared(curve);
        ts::end(s);
    ...
    fun test_post_comment_succeeds() {
        ...
        bonding_curve::post_comment(
            &mut curve, fee, string::utf8(b"great token"), ts::ctx(&mut s),
        );
        assert!(bonding_curve::protocol_fees_pending(&curve) == proto_before + COMMENT_FEE, 1800);
        ts::return_shared(curve);
        ts::end(s);
    }
    ...
    fun test_post_comment_wrong_fee_rejected() {
        ...
        bonding_curve::post_comment(
            &mut curve, bad_fee, string::utf8(b"hi"), ts::ctx(&mut s),
        );
        ts::return_shared(curve);
        ts::end(s);
    }
    ...
    fun test_empty_comment_rejected() {
        ...
        let fee = mint_sui(COMMENT_FEE, &mut s);
        bonding_curve::post_comment(
            &mut curve, fee, string::utf8(b""), ts::ctx(&mut s),
        );
        ts::return_shared(curve);
        ts::end(s);
    }
    ...
    fun test_comment_too_long_rejected() {
        ...
        bonding_curve::post_comment(
            &mut curve, fee, long_text, ts::ctx(&mut s),
        );
        ts::return_shared(curve);
        ts::end(s);
    }
    ...
    fun test_lock_tokens_creates_vesting_lock() {
        ...
        let clk = clock::create_for_testing(ts::ctx(&mut s));
        bonding_curve::lock_tokens(&curve, tokens, 0, D_7D, &clk, ts::ctx(&mut s)); // cliff 7d
        clock::destroy_for_testing(clk);
        ts::return_shared(curve);
    ...
    fun test_cliff_releases_nothing_before_end() {
        ...
        bonding_curve::lock_tokens(&curve, tokens, 0, D_30D, &clk, ts::ctx(&mut s));
        clock::destroy_for_testing(clk);
        ts::return_shared(curve);
    ...
    fun test_cliff_claim_after_end_releases_all() {
        ...
        bonding_curve::lock_tokens(&curve, tokens, 0, D_7D, &clk0, ts::ctx(&mut s));
        ...
        let claimed = bonding_curve::claim_vested(&mut lock, &clk, ts::ctx(&mut s));
        assert!(coin::value(&claimed) == 1_000_000, 2020);
    ...
    fun test_linear_releases_half_at_midpoint() {
        ...
        let tokens = mint_token(1_000_000, &mut s);
        let clk0 = clock::create_for_testing(ts::ctx(&mut s));
        bonding_curve::lock_tokens(&curve, tokens, 1, D_30D, &clk0, ts::ctx(&mut s)); // linear
        clock::destroy_for_testing(clk0);
        ts::return_shared(curve);
    ...
    fun test_linear_claim_twice_tracks_claimed() {
        ...
        let tokens = mint_token(1_000_000, &mut s);
        let clk0 = clock::create_for_testing(ts::ctx(&mut s));
        bonding_curve::lock_tokens(&curve, tokens, 1, D_30D, &clk0, ts::ctx(&mut s));
        clock::destroy_for_testing(clk0);
        ts::return_shared(curve);
        ...
        let c1 = bonding_curve::claim_vested(&mut lock, &clk, ts::ctx(&mut s));
        assert!(coin::value(&c1) == 250_000, 2040);
        destroy(c1);

        // claim again at 75% -> should release the additional 50%
        clock::set_for_testing(&mut clk, (D_30D * 3) / 4);
        let c2 = bonding_curve::claim_vested(&mut lock, &clk, ts::ctx(&mut s));
        assert!(coin::value(&c2) == 500_000, 2041);
        destroy(c2);
    ...
    fun test_monthly_releases_in_steps() {
        ...
        bonding_curve::lock_tokens(&curve, tokens, 2, D_180D, &clk0, ts::ctx(&mut s));
        clock::destroy_for_testing(clk0);
        ts::return_shared(curve);
    ...
    fun test_claim_before_anything_vests_aborts() {
        ...
        let tokens = mint_token(1_000_000, &mut s);
        let clk0 = clock::create_for_testing(ts::ctx(&mut s));
        bonding_curve::lock_tokens(&curve, tokens, 0, D_30D, &clk0, ts::ctx(&mut s)); // cliff
        clock::destroy_for_testing(clk0);
        ts::return_shared(curve);
        ...
        let c = bonding_curve::claim_vested(&mut lock, &clk, ts::ctx(&mut s));
        destroy(c);
        clock::destroy_for_testing(clk);
        ts::return_shared(lock);
        ts::end(s);
    ...
    fun test_non_beneficiary_cannot_claim() {
        ...
        let tokens = mint_token(1_000_000, &mut s);
        let clk0 = clock::create_for_testing(ts::ctx(&mut s));
        bonding_curve::lock_tokens(&curve, tokens, 1, D_7D, &clk0, ts::ctx(&mut s));
        clock::destroy_for_testing(clk0);
        ts::return_shared(curve);
    ...
    fun test_invalid_vest_mode_rejected() {
        ...
        let tokens = mint_token(1_000_000, &mut s);
        let clk = clock::create_for_testing(ts::ctx(&mut s));
        bonding_curve::lock_tokens(&curve, tokens, 9, D_7D, &clk, ts::ctx(&mut s)); // mode 9 invalid
        clock::destroy_for_testing(clk);
        ts::return_shared(curve);
        ts::end(s);
    }
    ...
    fun test_invalid_vest_duration_rejected() {
        ...
        let tokens = mint_token(1_000_000, &mut s);
        let clk = clock::create_for_testing(ts::ctx(&mut s));
        bonding_curve::lock_tokens(&curve, tokens, 0, 12345, &clk, ts::ctx(&mut s)); // bad duration
        clock::destroy_for_testing(clk);
        ts::return_shared(curve);
        ts::end(s);
    }
    ...
    fun test_monthly_under_30_days_rejected() {
        ...
        bonding_curve::lock_tokens(&curve, tokens, 2, D_7D, &clk, ts::ctx(&mut s));
        clock::destroy_for_testing(clk);
        ts::return_shared(curve);
        ts::end(s);
    }
    ...
    fun test_zero_amount_lock_rejected() {
        ...
        let clk = clock::create_for_testing(ts::ctx(&mut s));
        bonding_curve::lock_tokens(&curve, tokens, 0, D_7D, &clk, ts::ctx(&mut s));
        clock::destroy_for_testing(clk);
        ts::return_shared(curve);
        ts::end(s);
    }
